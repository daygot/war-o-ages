/* War O' Ages — shared components (exports to window) */

const WOA = window.WOA;
...
function BannerIcon({ icon, size = 26, color = 'currentColor', strokeWidth = 1.7 }) {
...
function Crest({ fig, size = 54, dashed = false, placeholderAbbr, ink, dim = false, pos }) {
...
function StatRow({ fig, compact = false }) {
  return (
...
// ── Figure card (full) ────────────────────────────────────────
function FigureCard({ fig, positionName, style, animate, pos }) {
  if (!fig) return null;
  return (
...
function FigureRow({ fig, positionAbbr, hidden, animate, delay = 0, pos }) {
  if (hidden || !fig) {
    return (
...
// ── Wax seal (grade) ──────────────────────────────────────────
function WaxSeal({ grade, size = 86, animate }) {
  const s = size;
  return (
...
const IdeologyIcon = BannerIcon;

Object.assign(window, { Crest, Tag, StatRow, FigureCard, FigureRow, WaxSeal, Banner, BannerIcon, IdeologyIcon, shade, hexA });
